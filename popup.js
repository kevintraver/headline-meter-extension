document.addEventListener('DOMContentLoaded', function () {
  ...
  async function analyzeUrl(url, forceRefresh = false) {
    ...
    try {
      // First check if we have cached results (unless forced refresh)
      ...
      const runId =
        runData.runId ||
        runData.id ||
        (typeof runData === 'string' ? runData : null)
      ...
      const startResponse = await fetch(
        `${baseUrl}/api/workflows/${workflowId}/start-async?runId=${runId}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (!startResponse.ok) {
        ...
        throw new Error(
          `Failed to start workflow: ${startResponse.status} - ${errorText}`
        )
      }
      ...
      if (
        response &&
        response.payload &&
        response.payload.workflowState &&
        response.payload.workflowState.result &&
        response.payload.workflowState.result.output
      ) {
